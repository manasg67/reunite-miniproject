...
const formSchema = z.object({
  ...
  last_known_longitude: z.string().optional(),
  aadhaar_number: z.string().length(12),
  documents: z.array(
    z.object({
...
const ReportMissing = () => {
  ...
  const handleLocationSelect = (location) => {
    form.setValue("last_seen_location", location.displayName);
    form.setValue("last_known_latitude", Number(parseFloat(location.latitude)).toFixed(3));
    form.setValue("last_known_longitude", Number(parseFloat(location.longitude)).toFixed(3));
  };
  ...
  const onSubmit = async (data) => {
    try {
      ...
      setSubmitStatus(t('report_missing.status.processing_data'));
      ...
      // Handle documents
      setSubmitStatus(t('report_missing.status.uploading_documents'));
  ...
  const handleFamilyMemberSelect = (memberId) => {
    ...
    if (member) {
      ...
      form.setValue("date_of_birth", member.date_of_birth);
      form.setValue("gender", member.gender);
      form.setValue("blood_group", member.blood_group);
      form.setValue("nationality", member.nationality);
      form.setValue("height", member.height);
      form.setValue("weight", member.weight);
      form.setValue("complexion", member.complexion);
      form.setValue("identifying_marks", member.identifying_marks);
      if (member.physical_attributes) {
        form.setValue("physical_attributes.hair_color", member.physical_attributes.hair_color);
        form.setValue("physical_attributes.eye_color", member.physical_attributes.eye_color);
        form.setValue("physical_attributes.build", member.physical_attributes.build);
      }
    }
  };
  ...
  return (
    <div className="min-h-full bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 mt-16">
      {isSubmitting && <LoadingOverlay message={submitStatus} />}
      <div className="max-w-4xl mx-auto">
        ...
        <Card className="shadow-lg">
          ...
          <div className="flex items-center space-x-2 mb-4">
            ...
            >
              Is the missing person a family member?
            </label>
          </div>
            ...
            <div className="mb-6">
              ...
              >
                <SelectTrigger className="w-full">
                  <SelectValue placeholder="Select a family member to auto-fill details" />
                </SelectTrigger>
                <SelectContent>
                        ...
                        <FormItem>
                          ...
                          <FormControl>
                            <Input
                              ...
                              step="0.000001"
                              {...field}
                              className="border-gray-200 hidden focus:ring-blue-500 focus:border-blue-500"
                              disabled
                            />
                          </FormControl>
                          <FormMessage />
